...
def load_ontology_from_file(io_helper, path):
...
def run_checks(ont_or_file, ontology_details, params):
    ...
    good_format = ontology_details['good_format']
    namespace = ontology_details['namespace']
    report = ontology_details['report']

    contact_schema = params['contact_schema']
    domain_map = params['domain_map']
    license_schema = params['license_schema']
    ontology_dir = params['ontology_dir']
    ro_props = params['ro_props']
    robot_gateway = params['robot_gateway']
...
def exec_checks(namespace, ont_or_file, ontology_details, params):
    data = ontology_details['data']
    big = ontology_details['big']
    ...
    try:
        ...
        if big:
            ...
            if namespace != 'gaz':
                ...
                ontology_details['report'] = report_obj.get_report()
                ontology_details['good_format'] = report_obj.get_good_format()
        else:
            robot_gateway = params['robot_gateway']
            io_helper = params['io_helper']
            if ont_or_file:
                # Ontology is not None
                print('Running ROBOT report on {0}...'.format(namespace), flush=True)
                ontology_details['report'] = report_utils.run_report(robot_gateway, io_helper, ont_or_file)

        # run each principle check
        return run_checks(ont_or_file, ontology_details, params)
...
def save_results(ontology, check_map, ontology_dir):
    ...
    save_data = {'namespace': ontology['namespace'], 'version': ontology['version_iri'],
                 'date': date.strftime('%Y-%m-%d'),
                 'summary': {'status': summary, 'comment': summary_comment}, 'results': all_checks}

    outfile = '/'.join([ontology_dir, 'dashboard.yml'])
...
def run(namespace, ontology_file, registry, license_schema, contact_schema, ro_file, outdir):
    """
    :param str namespace: namespace of ontology to check
    :param str ontology_file: path to ontology to check
    ...
    if not big:
        ...
        ont_or_file = load_ontology_from_file(io_helper, ontology_file)
    ...
    # Params for checking
    params = {'contact_schema': contact_schema,
    ...
    check_map = exec_checks(namespace, ont_or_file, ontology_details, params)
    save_results(ontology_details, check_map, ontology_dir)
    gc.collect()
    sys.exit(0)
...
if __name__ == '__main__':
    ...
    run(ns, args.ontology, args.registry, json.load(args.license), json.load(args.contact), args.relations, args.outdir)
